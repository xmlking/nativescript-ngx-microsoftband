...
@Component({
    ...
    template: `
      <StackLayout minHeight="100" class="sumo0" [row]="row" [col]="col" (tap)="toggle(iconLabel)" >
        ...
        <Label row="3" [text]="sensorData?.brightness" horizontalAlignment="center" class="font-weight-normal"></Label>
      </StackLayout>
    `,
...
export class AmbientLightComponent implements OnInit, OnDestroy {
    ...
    public sensorData: AmbientLightData;
    ...
    private startSensorUpdates() {
        this.msband.requestUserConsent((isGranted) => {
            if (isGranted) {
                this.sensorSub = this.msband.ambientLight$.subscribe(
