...
@Component({
    ...
    template: `
      <StackLayout minHeight="100" class="trendygreen" [row]="row" [col]="col" (tap)="toggle(iconLabel)" >
        ...
        <Label row="4" [text]="sensorData?.totalSteps" horizontalAlignment="center" class="font-weight-normal"></Label>
      </StackLayout>
    `,
...
export class PedometerComponent implements OnInit, OnDestroy {
    ...
    public sensorData: PedometerData;
    ...
    private startSensorUpdates() {
        this.msband.requestUserConsent((isGranted) => {
            if (isGranted) {
                this.sensorSub = this.msband.pedometer$.subscribe(
                    (data: PedometerData) => {
                        this.zone.run(() => {
